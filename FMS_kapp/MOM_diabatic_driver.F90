...
subroutine diabatic_ALE_legacy(u, v, h, tv, BLD, fluxes, visc, ADp, CDp, dt, Time_end, &
  ...
  real :: Kd_add_raw, Kd_add_pos, Kd_add_neg
  ...
  logical :: showCallTree ! If true, show the call tree
  integer :: i, j, k, is, ie, js, je, Isq, Ieq, Jsq, Jeq, nz, Kint
  logical :: additive_conflict !if true, shear energy coupling and ePBL addictive option conflicts, error and reset
  real    :: cap_max,cap, zsum !used to evaluate if shear needs to shutdown and the layer where it needs
  real, dimension(SZI_(G),SZJ_(G))  :: KS_cap_m !to evaulate where shear energy has been used by ePBL or not
  logical, dimension(SZI_(G),SZJ_(G),SZK_(GV)+1) :: osbl_mask  !to evaulate where shear energy has been used by ePBL or not
  real, dimension(SZI_(G),SZJ_(G),SZK_(GV)+1) :: w_kd !weighted function for smooth the Kd_interface profile
  real :: z0_kd ,z1_kd,z_top_kd,z_cen_kd,s_kd ! distance variables for calculating weighted function 
  logical :: IS_KS_weight = .false.
  real :: W_ratio = 0.8,p_sharp=3.5
  ...
  if (CS%debug) then
    ...
    call hchksum(ent_s, "after calc_entrain ent_s", G%HI, haloshift=0, scale=GV%H_to_MKS)
  endif
  ...
  if (CS%use_energetic_PBL) then
    ...
    call energetic_PBL(h, u_h, v_h, tv, fluxes, visc, dt, Kd_ePBL, G, GV, US, &
                       ...
                       KS_cap_m_out = KS_cap_m, IS_KS_weight=IS_KS_weight,W_ratio=W_ratio)                       
    ...
    cap_max=maxval(KS_cap_m(is:ie,js:je))
    ! EPBL_IS_ADDITIVE and shearshut down cannot be the same
    additive_conflict = (CS%ePBL_is_additive .and. cap_max > 0.0)

    if (additive_conflict) then
      call MOM_error(FATAL, &
        "Inconsistent settings: EPBL_IS_ADDITIVE=.true. but OSBL shear shutdown was requested "// &
        "(KS_shutdown_cap_m>0). To proceed, either set EPBL_IS_ADDITIVE=.false. to enable shear-energy "// &
        "coupling and OSBL shutdown, OR keep EPBL_IS_ADDITIVE=.true. and set "// &
        "EPBL_USE_SHEAR_ENERGY_COUPLING=.false. and EPBL_SHUTDOWN_SHEAR_IN_ML=.false.")
    end if
    w_kd(:,:,:)=0
    osbl_mask(:,:,:) = .false.
    do j=js,je; do i=is,ie
      ...
      if (cap > 0.0) then
        zsum = 0.0
        z0_kd   = W_ratio*cap      ! start of taper (top 70% stays at 1)
        z1_kd   = cap           ! end of taper (weight -> 0 at cap)
        do K=2,nz
          z_top_kd = zsum
          z_cen_kd = z_top_kd + 0.5*dz(i,j,K-1)
          if (zsum < cap) then
             osbl_mask(i,j,K) = .true.
              if (z_cen_kd <= z0_kd) then
                w_kd(i,j,K) = 1.0
              else if (z_cen_kd >= z1_kd) then
                w_kd(i,j,K) = 0.0
              else
                p_sharp=3.5
                s_kd = 1.0 - ((z_cen_kd - z0_kd) / max(z1_kd - z0_kd, 1.0e-12))**p_sharp
               ! s_kd = (z1_kd - z_cen_kd) / max(z1_kd - z0_kd, 1.0e-12)   ! map [z0,z1] -> [1,0]
                s_kd = max(0.0, min(1.0, s_kd))
                w_kd(i,j,K) = s_kd*s_kd*(3.0 - 2.0*s_kd)           ! C1 smoothstep
              end if
          endif
          zsum = zsum + dz(i,j,K-1)    ! consisten with  cap definition: dz, dz(i,j,K-1)
        end do
      end if
    end do; end do
    if (.not. IS_KS_weight)   w_kd(:,:,:)=1    
    ! Augment the diffusivities and viscosity due to those diagnosed in energetic_PBL.
    do K=2,nz ; do j=js,je ; do i=is,ie
      Kd_add_raw=0.0    
      if (CS%ePBL_is_additive) then
        Kd_add_raw = Kd_ePBL(i,j,K)
        visc%Kv_shear(i,j,K) = visc%Kv_shear(i,j,K) + CS%ePBL_Prandtl*Kd_ePBL(i,j,K)
      else
            if (associated(visc%Kd_shear) .and. osbl_mask(i,j,K) .and. Kd_int(i,j,K) >= visc%Kd_shear(i,j,K)) then
                Kd_add_raw =w_kd(i,j,K)*( Kd_ePBL(i,j,K) -visc%Kd_shear(i,j,K))+ &
                         (1-w_kd(i,j,K))*(max(Kd_ePBL(i,j,K) - visc%Kd_shear(i,j,K), 0.0))
                visc%Kv_shear(i,j,K) = max(visc%Kv_shear(i,j,K), CS%ePBL_Prandtl*Kd_ePBL(i,j,K))
            else   
               Kd_add_raw = max(Kd_ePBL(i,j,K) - visc%Kd_shear(i,j,K), 0.0)
               visc%Kv_shear(i,j,K) = max(visc%Kv_shear(i,j,K), CS%ePBL_Prandtl*Kd_ePBL(i,j,K))
            endif
      endif
      ! --- split increment: bookkeeping (both signs) vs tendencies (non-negative only) ---
      Kd_add_pos  = max(Kd_add_raw, 0.0)   ! contributes to tendencies + Kd_int
      Kd_add_neg  = min(Kd_add_raw, 0.0)   ! removes shear from Kd_int only
      Kd_add_here = Kd_add_pos             ! keep your downstream name for tendencies      

      Ent_int = Kd_add_raw * dt / (0.5*(dz(i,j,k-1) + dz(i,j,k)) + dz_neglect)
      ent_s(i,j,K) = max((ent_s(i,j,K) + Ent_int),0.0)
      Kd_int(i,j,K) =max((Kd_int(i,j,K) + Kd_add_raw),0.0)
    ...
    if (CS%debug) then
      call hchksum(ent_t, "after ePBL ent_t", G%HI, haloshift=0, scale=GV%H_to_MKS)
      call hchksum(ent_s, "after ePBL ent_s", G%HI, haloshift=0, scale=GV%H_to_MKS)
  ...
  if (associated(tv%T)) then
    ...
    if (CS%debug) then
      call hchksum(ent_t, "before triDiagTS ent_t ", G%HI, haloshift=0, scale=GV%H_to_MKS)
      call hchksum(ent_s, "before triDiagTS ent_s ", G%HI, haloshift=0, scale=GV%H_to_MKS)
    endif
...
subroutine diabatic_ALE(u, v, h, tv, BLD, fluxes, visc, ADp, CDp, dt, Time_end, &
  ...
  logical :: additive_conflict !if true, shear energy coupling and ePBL addictive option conflicts, error and reset
  real    :: cap_max,cap, zsum !used to evaluate if shear needs to shutdown and the layer where it needs
  real, dimension(SZI_(G),SZJ_(G))  :: KS_cap_m
  logical, dimension(SZI_(G),SZJ_(G),SZK_(GV)+1) :: osbl_mask  !to evaulate where shear energy has been used by ePBL or not
  real, dimension(SZI_(G),SZJ_(G),SZK_(GV)+1) :: w_kd !weighted function for smooth the Kd_interface profile
  real :: z0_kd ,z1_kd,z_top_kd,z_cen_kd,s_kd ! distance variables for calculating weighted function 
  logical :: IS_KS_weight = .false.
    ...
    do K=2,nz ; do j=js,je ; do i=is,ie
      ...
      if (CS%ePBL_is_additive) then
        ...
        visc%Kv_shear(i,j,K) = visc%Kv_shear(i,j,K) + CS%ePBL_Prandtl*Kd_ePBL(i,j,K)
            ...
            if (associated(visc%Kd_shear) .and. osbl_mask(i,j,K) .and. Kd_heat(i,j,K) >= visc%Kd_shear(i,j,K) &
                    .and. Kd_salt(i,j,K) >= visc%Kd_shear(i,j,K)) then
                Kd_add_raw =w_kd(i,j,K)*( Kd_ePBL(i,j,K) -visc%Kd_shear(i,j,K))+ &
                         (1-w_kd(i,j,K))*(max(Kd_ePBL(i,j,K) - visc%Kd_shear(i,j,K), 0.0))
                visc%Kv_shear(i,j,K) = max(visc%Kv_shear(i,j,K), CS%ePBL_Prandtl*Kd_ePBL(i,j,K))           
           else
      ...
      Kd_heat(i,j,K) = max((Kd_heat(i,j,K) + Kd_add_pos+Kd_add_neg),0.0)
      Kd_salt(i,j,K) = max((Kd_salt(i,j,K) + Kd_add_pos+Kd_add_neg),0.0)
    enddo ; enddo ; enddo
...
!    (5) Possibly splits buffer layer into two isopycnal layers (when using isopycnal coordinate)
  ...
  if (CS%use_kappa_shear .or. CS%use_CVMix_shear) then
    if ((CS%ML_mix_first > 0.0) .or. CS%use_geothermal) then
      ...
      if (CS%debug) then
        call hchksum(eaml, "after find_uv_at_h eaml", G%HI, scale=GV%H_to_MKS)
        call hchksum(ebml, "after find_uv_at_h ebml", G%HI, scale=GV%H_to_MKS)
      endif
    else
      call find_uv_at_h(u, v, h, u_h, v_h, G, GV, US)
    endif
  ...
  if (CS%debug) then
    ...
    call MOM_thermovar_chksum("after calc_entrain ", tv, G, US)
    call MOM_state_chksum("after calc_entrain ", u, v, h, G, GV, US, haloshift=0)
    call hchksum(ea, "after calc_entrain ea", G%HI, haloshift=0, scale=GV%H_to_MKS)
    call hchksum(eb, "after calc_entrain eb", G%HI, haloshift=0, scale=GV%H_to_MKS)
  endif
  ...
  if (CS%use_bulkmixedlayer) then
    ...
    if ((CS%ML_mix_first > 0.0) .or. CS%use_geothermal) then
      ...
      enddo ; enddo ; enddo
      if (CS%debug) then
        call hchksum(ea, "after ea = ea + eaml", G%HI, haloshift=0, scale=GV%H_to_MKS)
        call hchksum(eb, "after eb = eb + ebml", G%HI, haloshift=0, scale=GV%H_to_MKS)
      endif
    endif
    ...
    if (associated(tv%T)) then
      ...
      if (CS%debug) then
        call hchksum(ea, "before triDiagTS ea ", G%HI, haloshift=0, scale=GV%H_to_MKS)
        call hchksum(eb, "before triDiagTS eb ", G%HI, haloshift=0, scale=GV%H_to_MKS)
      endif
      call cpu_clock_begin(id_clock_tridiag)
  ...
  if (CS%debug) then
    call MOM_state_chksum("after mixed layer ", u, v, h, G, GV, US, haloshift=0)
    call MOM_thermovar_chksum("after mixed layer ", tv, G, US)
    call hchksum(ea, "after mixed layer ea", G%HI, scale=GV%H_to_MKS)
    call hchksum(eb, "after mixed layer eb", G%HI, scale=GV%H_to_MKS)
  endif
...
! mixed layer turbulence is applied elsewhere.
  if (CS%use_bulkmixedlayer) then
    if (CS%debug) then
      call hchksum(ea, "before net flux rearrangement ea", G%HI, scale=GV%H_to_MKS)
      call hchksum(eb, "before net flux rearrangement eb", G%HI, scale=GV%H_to_MKS)
    endif
    ...
    enddo
    if (CS%debug) then
      call hchksum(ea, "after net flux rearrangement ea", G%HI, scale=GV%H_to_MKS)
      call hchksum(eb, "after net flux rearrangement eb", G%HI, scale=GV%H_to_MKS)
    endif
  endif
...
! Initialize halo regions of ea, eb, and hold to default values.
  ...
  if (CS%debug) then
    ...
    call hchksum(ea, "before u/v tridiag ea", G%HI, scale=GV%H_to_MKS)
    call hchksum(eb, "before u/v tridiag eb", G%HI, scale=GV%H_to_MKS)
    call hchksum(hold, "before u/v tridiag hold", G%HI, scale=GV%H_to_MKS)
  endif
  call cpu_clock_begin(id_clock_tridiag)
