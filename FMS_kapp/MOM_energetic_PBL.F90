...
type, public :: energetic_PBL_CS ; private
  ...
  !/ Energy-based coupling with kappa_shear
  logical :: use_shear_energy_coupling = .false.  ! switch: shear energy_coupling
  logical :: shutdown_shear_in_ML      = .true.   ! shutdown Kd_shear within MLD when shear energy is used for Me for ePBL
  logical :: warn_if_no_kappa_shear    = .true.   ! when kappa_shear is false but energy coupling is enabled, warnning is on
  logical :: LT_rescale_by_shear = .false.   ! LT will be rescaled by ePBL results
  real    :: LT_enhance_top = 1.0
  character(len=32) :: Shear_LT_mode = 'preserve_fraction' !rescaled by mstar_LT/mstar_total for ePBL and shear-energy
  real    :: LT_rescale_gamma    = 0.5       !recommended 0.5 when using "energy-gamma', 1 is recommended using preserve_fraction
  logical :: IS_KS_weight            =.false.  !weighted function apply to Kd_interface profile
  real    :: W_ratio            =0.8          !ratio top layer keeps unsmoothed, unchanged
  real :: Shear_alpha           =-1! <0 => auto (Ri-based); >=0 => prescribed alpha in [0,1] for ME integral in shear energy
  real :: Shear_ePBL_cap = 30 !the cap for mstar_shear/mstar_ePBL, which means enhance factor should be smaller than this
  ...
  integer :: id_OSBL_Me_KS = -1, id_OSBL_Conv_KS = -1, id_E_ePBL = -1, id_E_shear = -1
  integer :: id_Hbl_est = -1,   id_used_shear_flag = -1
  integer :: id_mstar_ePBL = -1, id_mstar_shear = -1 
  integer :: id_LT_scale_raw = -1 , id_LT_scale_top = -1   
  !>@}
...
type, public :: ePBL_column_diags ; private
  ...
  real :: LT_scale_raw     ! final scale before LT_enhance_top
  real :: LT_scale_top     ! final scale after  LT_enhance_top  
...
subroutine energetic_PBL(h_3d, u_3d, v_3d, tv, fluxes,visc, dt, Kd_int, G, GV, US, CS, &
                         stoch_CS, dSV_dT, dSV_dS, TKE_forced, buoy_flux, Waves, KS_cap_m_out,IS_KS_weight,W_ratio)
  ...
  real, optional, intent(out), dimension(:,:)   :: KS_cap_m_out  !< [i,j] OSBL depth to shut down shear, 0 indicates not shutdown
  logical, optional, intent(out) :: IS_KS_weight
  real, optional, intent(out) :: W_ratio
!    This subroutine determines the diffusivities from the integrated energetics
!  mixed layer model.  It assumes that heating, cooling and freshwater fluxes
!  have already been applied.  All calculations are done implicitly, and there
!  is no stability limit on the time step.
...
!      mstar = 1.25, nstar = 0.4, TKE_decay = 0.0, conv_decay = 0.0
  ...
  logical, save :: warned_noKS = .false.       !only warn once
  ...
  if (present(KS_cap_m_out)) KS_cap_m_out = 0.0
  ...
  if (CS%use_LT) then
          diag_LT_scale_raw=1.0
          diag_LT_scale_top=1.0
  endif  
  ...
  do j=js,je
    ...
    do i=is,ie ; if (G%mask2dT(i,j) > 0.0) then
      ...
      if (present(IS_KS_weight)) IS_KS_weight = CS%IS_KS_weight
      if (present(W_ratio))      W_ratio      = CS%W_ratio
      ...
      if (want_energy_coupling .and. have_KS) then
        call compute_shear_energy_1col( T0, S0, dSV_dT_1d, dSV_dS_1d, dz, &
                                        visc%Kd_shear(i,j,:), Me_shear, Conv_shear, &
                                        Hbl_est, GV, US )
        eCD%Hbl_est  = Hbl_est   !roughly estimate of MLD before rewritten by ePBL                        
      else
        Me_shear   = 0.0
        Conv_shear = 0.0
        Hbl_est = 0.0;
             ...
             call ePBL_column(h, dz, u, v, T0, S0, dSV_dT_1d, dSV_dS_1d, SpV_dt, TKE_forcing, B_flux, absf, &
                              ...
                              TKE_gen_stoch=stoch_CS%epbl1_wts(i,j), TKE_diss_stoch=stoch_CS%epbl2_wts(i,j), &
                                 use_shear_energy=(want_energy_coupling .and. have_KS), &
                                  Me_shear_in=Me_shear, Conv_shear_in=Conv_shear,Kd_shear_col=visc%Kd_shear(i,j,:))
      ...
      if (CS%use_LT) then
              diag_LT_scale_raw(i,j)=eCD%LT_scale_raw
              diag_LT_scale_top(i,j)=eCD%LT_scale_top
      endif      
      ml_cap = max(0.0, min(Hbl_est, sum(dz)))
!  ! --- shutdown kappa-shear inside OSBL if this column used shear energy ---      
      if (present(KS_cap_m_out)) then
          if (CS%use_shear_energy_coupling .and. CS%shutdown_shear_in_ML .and. have_KS .and. eCD%used_shear_flag >= 0.5) then
               KS_cap_m_out(i,j) = ml_cap
          else
               KS_cap_m_out(i,j) = 0.0
          endif
      endif
  ...
  if (CS%id_LT_scale_raw > 0) call post_data(CS%id_LT_scale_raw, diag_LT_scale_raw, CS%diag)
  if (CS%id_LT_scale_top > 0) call post_data(CS%id_LT_scale_top, diag_LT_scale_top, CS%diag)  
...
subroutine ePBL_column(h, dz, u, v, T0, S0, dSV_dT, dSV_dS, SpV_dt, TKE_forcing, B_flux, absf, &
                       ...
                       use_shear_energy, Me_shear_in, Conv_shear_in,Kd_shear_col)
...
! kappa shear energy coupling
  ...
  real, dimension(:), optional, intent(in) :: Kd_shear_col  ! size SZK_(GV)+1 (interfaces)
...
! mixing.
  ...
  real :: E_shear, u3, mstar_e_local, E_ePBL_local,Me_shear_loc, Conv_shear_loc,Hbl_est, msafe
  real :: lt_scale, E_used, denom, ratio_lt !where LT will be rescaled based on energy         
  real :: mstar_LT0, tmp_e_local, scale_need,base0,mstar_shear,used_shear_flag,LA_tmp
  real :: Hcap, alpha, Rib, Ric, epsU, dU2, dRho, g_local
  real :: zsum, hmin
  logical :: mstar_shear_ok, thick_ok
  integer :: k_H
  real :: Ri_lo, Ri_hi, a_lo, a_hi
...
! interface.
  ...
  do OBL_it=1,CS%Max_MLD_Its
    ...
    if (.not. OBL_converged) then
      ...
      if (.not. CS%use_shear_energy_coupling) then      
         if (CS%Use_LT) then
          ...
          eCD%used_shear_flag = 0.0
           ...
           if (present(use_shear_energy) .and. use_shear_energy .and. &
               ...
               Me_shear_loc=Me_shear_in
               Conv_shear_loc=Conv_shear_in
               if (present(Kd_shear_col)) then
                    if (CS%Shear_alpha > 0.0) then
                         alpha = CS%Shear_alpha
                    else
                         Ric     = 0.25
                         epsU    = 1.0e-6
                         g_local = 9.81 * US%L_to_m / US%T_to_s**2
                         ! locate k_H so sum(dz(1..k_H)) ~ |MLD_guess|
                         zsum = 0.0 ; k_H = 1
                         do k=1,nz-1
                           if (zsum + dz(k) >= abs(MLD_guess)) exit
                           zsum = zsum + dz(k)
                           k_H  = k
                         enddo

                         ! bulk Ri using surface vs. k_H (simple & robust enough for your need)
                         dU2  = (u(k_H)-u(1))**2 + (v(k_H)-v(1))**2
                         dRho = GV%Rho0 * ( 0.5*(dSV_dT(1)+dSV_dT(k_H))*(T0(1)-T0(k_H)) + &
                                            0.5*(dSV_dS(1)+dSV_dS(k_H))*(S0(1)-S0(k_H)) ) * g_local
                         Rib  = (dRho/GV%Rho0) * (abs(MLD_guess)/max(dU2, epsU))
                         ! map bulk Ri -> alpha in [0.85, 1.00] (piecewise-linear clamp around classic ~0.25)
                         Ri_lo = 0.15 ; Ri_hi = 0.35   ! transition window
                         a_lo  = 0.8 ; a_hi = 1.00    ! gentle shrink only
                         alpha = a_lo + (a_hi - a_lo) * max(0.0, min(1.0, (Rib - Ri_lo)/(Ri_hi - Ri_lo)))
                    endif
                    ! final cap and call
                    Hcap  = max(0.0, min(alpha * abs(MLD_guess), sum(dz)))
                    call compute_shear_energy_1col( T0, S0, dSV_dT, dSV_dS, dz, Kd_shear_col, &
                                    Me_shear_loc, Conv_shear_loc, Hbl_est, GV, US, MLD_in=Hcap)
                    eCD%Hbl_est = Hbl_est   !roughly estimate of MLD rewritten by MLD_guess
               endif
               eCD%OSBL_Me_KS   = Me_shear_loc         ! ≤ 0, [m^3 s^-3]
               eCD%OSBL_Conv_KS = Conv_shear_loc       ! ≥ 0, [m^3 s^-3]
               E_shear= max(0.0, -Me_shear_loc - CS%nstar * Conv_shear_loc)
               eCD%E_shear      = E_shear              ! [m^3 s^-3]
               mstar_shear  = E_shear / max(u3, tiny(1.0))
               eCD%mstar_shear  = mstar_shear               
               msafe = eCD%mstar_shear
               mstar_shear_ok = ((msafe == msafe) .and. (abs(msafe) < 1.0e4) .and. (msafe > 0.0))
               hmin = 2.0 * dz(1)           ! 也可以用 3.0*dz(1) 更保守
               thick_ok = (abs(Hbl_est) >= hmin)
               if ((E_shear > E_ePBL_local) .and. (u3 > 1.0e-6) .and. mstar_shear_ok .and. thick_ok) then
                   mstar_total            = min(mstar_shear, CS%Shear_ePBL_cap*max(mstar_e_local, 1.0e-5))
                   used_shear_flag    = 1.0
               else
                    mstar_total = mstar_e_local
                    used_shear_flag    = 0.0
               endif
             eCD%used_shear_flag = used_shear_flag               
           else
             mstar_total = mstar_e_local
             eCD%OSBL_Me_KS        = 0.0
         ...
         !Calculate mstar_LT based on mstar_total that has been potential replaced by shear energy 
           if (CS%Use_LT) then
               call get_Langmuir_Number(LA, G, GV, US, abs(MLD_guess), u_star_mean, i, j, dz, Waves, &
                                        U_H=u, V_H=v)
            ! --- reference LT on ePBL base to form fraction f0 (no shear rescale) ---
               mstar_LT0 = 0.0
               tmp_e_local=mstar_e_local
               call mstar_Langmuir(CS, US, absf, B_flux, u_star, MLD_guess, La, &
                                   tmp_e_local, mstar_LT0, LA_tmp)
               lt_scale = 1.0
               if (CS%LT_rescale_by_shear) then
                select case (CS%Shear_LT_mode)
                 case ('energy_gamma')
                     if ((used_shear_flag>0.5).and. (mstar_LT0<=mstar_e_local) .and. &
                (mstar_LT0 < 0.8 * mstar_total) .and. (min(mstar_total,mstar_e_local)> 1.0e-5)  )  then                       
                       E_used  = mstar_total*u3
                       denom   = max(E_ePBL_local, 1.0e-12)
                       ratio_lt=E_used/denom
                       lt_scale = min(max(1.0, ratio_lt**CS%LT_rescale_gamma),CS%Shear_ePBL_cap)
                       if (.not.(lt_scale .eq. lt_scale)) lt_scale = 1.0
                     endif
                     eCD%LT_scale_raw=lt_scale
                     lt_scale = lt_scale * CS%LT_enhance_top
                     eCD%LT_scale_top = lt_scale
                     call mstar_Langmuir(CS, US, absf, B_flux, u_star, MLD_guess, La, &
                                             mstar_total, mstar_LT, LAmod, Rescale_LT=lt_scale)
                 case ('preserve_fraction')     !keep the mstar_LT/(mstar_ePBL) ratio the same
                     scale_need= 1.0
                     if ((used_shear_flag>0.5) .and. (mstar_LT0<=mstar_e_local) .and. &
              (mstar_LT0 < 0.8 * mstar_total) .and. (min(mstar_total,mstar_e_local)> 1.0e-5)) then                           
                           base0 = mstar_shear / max(mstar_e_local, 1.0e-5)
                           scale_need = min(max(base0**CS%LT_rescale_gamma,1.0),CS%Shear_ePBL_cap)
                           if (.not.(scale_need .eq. scale_need)) scale_need = 1.0
                     endif
                     eCD%LT_scale_raw= scale_need
                     scale_need = scale_need * CS%LT_enhance_top
                     eCD%LT_scale_top = scale_need
                     call mstar_Langmuir(CS, US, absf, B_flux, u_star, MLD_guess, La, &
                                         mstar_total, mstar_LT, LAmod, Rescale_LT=scale_need) ! updates mstar_total in-place
                 end select                
                else
                   call mstar_Langmuir(CS, US, absf, B_flux, u_star, MLD_guess, La, &
...
!This function calculate the Me energy for a single column from kappa shear using w'b',diapycnal buoyancy flux calculation is based on
!MOM_diagnose_KdWork.F90 and MOM_kappa_shear.F90
subroutine compute_shear_energy_1col(T0, S0, dSV_dT, dSV_dS, dz, Kd_shear_col, &
                                     Me_shear, Conv_shear, Hbl_est, GV, US,MLD_in)
  type(verticalGrid_type), intent(in)    :: GV     !< The ocean's vertical grid structure.
  type(unit_scale_type),   intent(in)    :: US     !< A dimensional unit scaling type
  real, dimension(SZK_(GV)), intent(in)  :: dz     !< The vertical distance across layers [Z ~> m].
  real, dimension(SZK_(GV)), intent(in)  :: T0     !< The initial layer temperatures [C ~> degC].
  ...
  real, dimension(SZK_(GV)), intent(in)  :: dSV_dS !< The partial derivative of in-situ specific
                                                   !! volume with salinity [R-1 S-1 ~> m3 kg-1 ppt-1].
  real, dimension(SZK_(GV)+1), intent(in) :: Kd_shear_col !< Shear-only diapycnal diffusivity
                                                            !!  at interfaces (zi) [Z^2 T^-1 ~> m^2 s^-1].
  real, intent(out) :: Me_shear   !< OSBL integral of NEGATIVE w′b′ (entrainment PE conversion),
                                  !!  units [Z^3 T^-3 ~> m^3 s^-3]; algebraically ≤ 0.
  real, intent(out) :: Conv_shear !< OSBL integral of POSITIVE w′b′ (convective buoyancy work),
                                  !!  units [Z^3 T^-3 ~> m^3 s^-3]; algebraically ≥ 0.
  real, intent(out) :: Hbl_est    !< Mixed-layer depth estimate by max |dθ/dz| on zl [Z ~> m].
  real, optional, intent(in) :: MLD_in  !< mixed layer depth estimated by ePBL [m] (use ePBL_MLD if present)
!  This subroutine compute the shear-driven buoyancy work within the surface boundary layer (OSBL) for
!  a single water column, using interface diffusivities from the kappa_shear scheme. The algorithm for estimating
!  the energy (OSBL-only) is similar with LES-informed Energy:
!    1) Estimate a mixed-layer depth Hbl_est by the depth of max |dθ/dz| (layer-centered θ).
!    2) Form interface N^2 from EOS derivatives: N^2 = (∂b/∂T ΔT + ∂b/∂S ΔS)/Δz,
!       with  ∂b/∂T ≈ g ρ0 ∂(1/ρ)/∂T  and  ∂b/∂S ≈ g ρ0 ∂(1/ρ)/∂S.
!    3) Compute interface buoyancy flux due to shear diffusion:  w′b′ = - N^2 * Kd_shear  (on zi).
!    4) Integrate over  0 ≥ z ≥ -Hbl_est  (top-down) using layer thickness to obtain:
!         Me_shear   = ∫_OSBL min(0, w′b′) dz    (entrainment PE conversion; NEGATIVE integral)
!         Conv_shear = ∫_OSBL max(0, w′b′) dz    (convective part; POSITIVE integral)
!
!  Sign & units:
!    - At interfaces:   w′b′ has units [Z^2 T^-3 ~> m^2 s^-3].
!    - After layer-thickness integration: Me_shear, Conv_shear have units [Z^3 T^-3 ~> m^3 s^-3].
!    - By convention here, Me_shear is the (algebraically) negative integral; users typically
!      feed  (-Me_shear - nstar*Conv_shear)/u*^3  to form a mechanical m* (nondimensional).
!
!  Notes:
!    - This routine is intentionally EOS-light by using precomputed dSV_dT/dSV_dS from the caller,
!      which are consistent with the model’s in-situ equation of state.
!    - Only the OSBL (z ≤ Hbl_est) contributes to the integrals.

  !local variables
  integer :: nz        !< Number of layers in the column (= size(dz))
  integer :: k, K_1,kk      !< Layer (k) and interface (K=k+1) indices
  ...
  real    :: dbdS      !< ∂b/∂S at an interface [Z T^-2 S^-1 ~> m s^-2 ppt^-1]
  real    :: dz_int    !< Interface thickness proxy = 0.5*(dz(k-1)+dz(k)) [Z ~> m]
  real    :: I_dz_int  !< 1/dz_int [Z^-1 ~> m^-1]
  real    :: dT, dS    !< Layer jumps across interface [C], [ppt]
  real, allocatable :: N2(:)      !< Buoyancy frequency squared on interfaces [T^-2 ~> s^-2]
  real, allocatable :: Bshear(:)  !< Interface w′b′ = -N^2 * Kd_shear [Z^2 T^-3 ~> m^2 s^-3]
  real, allocatable :: dTdz(:)    !< dθ/dz on layers [C Z^-1 ~> degC m^-1]
  real, allocatable :: dTdz_zi(:) !< Interpolated dθ/dz on interfaces [C Z^-1 ~> degC m^-1]
  real    :: zsum      !< Running depth from surface downward [Z ~> m]
  real    :: contrib   !< Layer-integrated contribution of w′b′ over dz(k) [Z^3 T^-3 ~> m^3 s^-3]
  real    :: v
  logical :: ks_ok     !check whether Kd_shear is NAN, negative or INF
  ! === Guard: validate Kd_shear_col; if bad -> zero energies and return ===
  
  ks_ok = .true.
  do kk = 1, size(Kd_shear_col)
    v = Kd_shear_col(kk)
   if ( (.not.(v == v))                              .or. &  ! NaN
       (.not.(abs(v) < huge(v)))                    .or. &  ! Inf/非有限
       (v < -1.0e-12)                               .or. &  ! 明显负值
        (v > 1.0) ) then ! 物理上限
    ks_ok = .false.
    exit
  ...
  if (.not. ks_ok) then
    Me_shear   = 0.0
    Conv_shear = 0.0
    ! 可选：给 Hbl_est 一个安全值（若其为 intent(out)/inout）
     Hbl_est = 0.0
    call MOM_mesg('compute_shear_energy_1col: invalid Kd_shear; energies set to zero.', 0) 
    return
  endif

  nz = size(dz) ; allocate(N2(nz+1), Bshear(nz+1), dTdz(nz), dTdz_zi(nz+1))
  g  = 9.81 * US%L_to_m / US%T_to_s**2   ! Gravity in model units (usually 9.81), in US normally L_to_m=1, T_to_s=1
  ! 1) Layer-centered gradient and interface interpolation of dθ/dz
  do k=1,nz-1
    dTdz(k) = (T0(k) - T0(k+1)) / dz(k+1)
  enddo
  dTdz(nz)   = dTdz(nz-1)
  dTdz_zi(1) = dTdz(1)
  do K_1=2,nz
  ...
  ! 2) Hbl estimate from max |dθ/dz|
  k_H = 1
  do k=2,nz
    if (abs(dTdz(k)) > abs(dTdz(k_H))) k_H = k
  enddo
  Hbl_est = 0.0
  do k=1,k_H
    Hbl_est = Hbl_est + dz(k)
  enddo
 if (present(MLD_in)) then
  Hbl_est=max(0.0, min(MLD_in, sum(dz)))
 endif
  ...
  do K_1=2,nz
    ...
    dT      = (T0(k-1)-T0(k))
    dS      = (S0(k-1)-S0(k))
    N2(K_1)   = I_dz_int * (dbdT*dT + dbdS*dS)
  !  Bshear(K_1) = - N2(K_1) * Kd_shear_col(K_1)
    Bshear(K_1) = -1 * Kd_shear_col(K_1) * &
              ( g * ( 0.5*(dSV_dT(k-1)+dSV_dT(k)) * (T0(k-1)-T0(k))/dz_int + &
                      0.5*(dSV_dS(k-1)+dSV_dS(k)) * (S0(k-1)-S0(k))/dz_int ) )
  enddo
  Bshear(1)    = 0.0
  Bshear(nz+1) = 0.0
  ...
  do k=1,nz-1
    ...
    if (contrib < 0.0) then
      ...
      Conv_shear = Conv_shear + contrib
    endif
    zsum = zsum + dz(k)
  enddo
  Me_shear  =  GV%Rho0 * Me_shear
  Conv_shear = GV%Rho0 * Conv_shear
end subroutine compute_shear_energy_1col
...
subroutine Mstar_Langmuir(CS, US, Abs_Coriolis, Buoyancy_Flux, UStar, BLD, Langmuir_Number, &
                          Mstar, MStar_LT, Convect_Langmuir_Number, Rescale_LT)
  type(energetic_PBL_CS), intent(in) :: CS    !< Energetic PBL control structure
  type(unit_scale_type), intent(in)  :: US    !< A dimensional unit scaling type
  real,                  intent(in)  :: Abs_Coriolis !< Absolute value of the Coriolis parameter [T-1 ~> s-1]
  real,                  intent(in)  :: Buoyancy_Flux !< Buoyancy flux [Z2 T-3 ~> m2 s-3]
  ...
  real,         optional, intent(in) :: Rescale_LT            ! multiplicative factor ≥ 1 (optional)
  ...
  real :: Ekman_Obukhov_un   ! The Ekman layer thickness divided by the Obukhov depth under unstable
                             ! conditions or 0 under stable conditions [nondim].
  real :: m0, mstar_LT_base  ! original mstar and mstar_LT before rescale by shear energy coupling
  real :: scale_LT           ! the mstar_LT scale by energy
  ...
  m0 = mstar
  mstar_LT_base = (enhance_mstar - 1.0)*m0 + mstar_LT_add
  scale_LT = 1.0
  if (present(Rescale_LT)) scale_LT = max(1.0, Rescale_LT)
  mstar_LT = mstar_LT_base * scale_LT
  mstar    = m0 + mstar_LT
...
!/2. Options related to setting MSTAR
  ...
  !---------------------------------------------------------------------------
  !  Options related to energy-based coupling (ignored if EPBL_MSTAR_SCHEME=CONSTANT)
  !---------------------------------------------------------------------------
  call get_param(param_file, mdl, "EPBL_USE_SHEAR_ENERGY_COUPLING", CS%use_shear_energy_coupling, &
                 "If true, use energy-dominant coupling: compare shear-derived Me/conv with ePBL estimate; "\
                 //"when shear dominates, set m* from shear energy and shutdown kappa_shear within MLD.", &
  ...
  call get_param(param_file, mdl, "IS_KS_WEIGHT", CS%IS_KS_weight, &
          "If true, apply weighted function on Kd_interface for smooth profile when shear energy shuts down", &
                 default=.false.,&
                 do_not_log=(CS%mstar_scheme==Use_Fixed_MStar .and. (.not. CS%use_shear_energy_coupling)))
  call get_param(param_file, mdl, "W_RATIO", CS%W_ratio, &
          "when apply weighted function on Kd_interface for smooth profile,top ratio of ML keeps unchange", &
                 units="nondim",default=0.8,&
                 do_not_log=(CS%mstar_scheme==Use_Fixed_MStar .and. (.not. CS%use_shear_energy_coupling)))
  call get_param(param_file, mdl, "SHEAR_EPBL_CAP", CS%Shear_ePBL_cap, &
               "when use shear energy ePBL coupling, the cap ratio for enhancement factor"\
       "normaly would be smaller than 50.0", &
               units="nondim", default=30.0,&
                 do_not_log=(CS%mstar_scheme==Use_Fixed_MStar .and. (.not. CS%use_shear_energy_coupling)))
  call get_param(param_file, mdl, "SHEAR_ALPHA", CS%Shear_alpha, &
               "If >=0, use this fixed alpha for KS energy cap: Hcap=alpha*MLD_guess. "\
               "If <0, alpha is computed from bulk Ri.", &
               units="nondim", default=-1.0)
  if (CS%Shear_alpha >= 0.0) then
    if (CS%Shear_alpha > 2.0) then
      call MOM_mesg('SHEAR_ALPHA > 2; clamping to 2.0', 0)
    endif
    if (CS%Shear_alpha < 0.0) then
      call MOM_mesg('SHEAR_ALPHA < 0; using Ri-based alpha instead', 0)
    endif
    CS%Shear_alpha = max(0.0, min(2.0, CS%Shear_alpha))
  endif
  ! If m* is fixed (CONSTANT scheme), energy-based coupling must be ignored.
  if (CS%mstar_scheme == Use_Fixed_MStar) then
    if (CS%use_shear_energy_coupling) then
...
!/ Turbulent velocity scale in mixing coefficient
  ...
  !shear energy coupling to get a good basedline and add LT effects on top of it
  call get_param(param_file, mdl, "EPBL_LT_RESCALE_BY_SHEAR", CS%LT_rescale_by_shear, &
  ...
  call get_param(param_file, mdl, "SHEAR_LT_MODE", CS%Shear_LT_mode, &
                 "Shear energy-Langmuir rescale mode: 'preserve_fraction' or 'energy_gamma'.", &
                 default="preserve_fraction", &
                 do_not_log=(.not. (CS%Use_LT .and. CS%use_shear_energy_coupling .and. CS%LT_rescale_by_shear)))
  if ( (CS%Shear_LT_mode /= 'preserve_fraction') .and.  (CS%Shear_LT_mode /= 'energy_gamma') ) then
    call MOM_error(FATAL, 'ePBL: SHEAR_LT_MODE must be "preserve_fraction" or "energy_gamma".')
  endif
  call get_param(param_file, mdl, "EPBL_LT_RESCALE_GAMMA", CS%LT_rescale_gamma, &
                       "Exponent gamma for LT rescale: used in 'energy_gamma' (E_used/E_ePBL)^gamma (0.2–0.6recommended)"\
               "and in 'preserve_fraction' as (target_full/mLTtmp)^gamma. (1 recommended).", &
                 units="nondim", default=0.5, &
                 do_not_log=(.not. (CS%Use_LT .and. CS%use_shear_energy_coupling)))
  if (CS%LT_rescale_by_shear .and. (CS%LT_rescale_gamma < 0.2 .or. CS%LT_rescale_gamma > 1.0)) then
     write(gamma_str,'(F6.3)') CS%LT_rescale_gamma
     call MOM_mesg('EPBL_LT_RESCALE_GAMMA outside 0.2-1.0; using value"'//trim(gamma_str)//'"',0)
  endif
  CS%LT_rescale_gamma = max(0.0, min(CS%LT_rescale_gamma, 1.0))
  call get_param(param_file, mdl, "LT_ENHANCE_TOP", CS%LT_enhance_top, &
          "Extra multiplicative factor applied to the final LT scale for hurricane conditions; 1.0 = no change.",&
          units="nondim", default=1.0, do_not_log=(.not. (CS%Use_LT)))
  CS%LT_enhance_top = max(1.0, CS%LT_enhance_top)
...
!/ Checking output flags
  ...
  if (CS%use_LT) then
    ...
    CS%id_LT_scale_raw = register_diag_field('ocean_model','ePBL_LT_scale_raw', diag%axesT1, &
        Time, 'Final LT scale before LT_enhance_top', units='nondim')
    CS%id_LT_scale_top = register_diag_field('ocean_model','ePBL_LT_scale_top', diag%axesT1, &
        Time, 'Final LT scale after LT_enhance_top', units='nondim')
  endif
  if (CS%use_shear_energy_coupling .and. kappa_shear_is_used(param_file)) then
    ...
    CS%id_E_ePBL = register_diag_field('ocean_model','E_ePBL',diag%axesT1,Time, &
      'Mechanical energy from ePBL baseline (m* u*^3)', 'W m-2', conversion=US%RZ3_T3_to_W_m2)
    CS%id_E_shear = register_diag_field('ocean_model','E_shear',diag%axesT1,Time, &
      'Mechanical energy implied by shear (-Me - n*Conv)', 'W m-2', conversion=US%RZ3_T3_to_W_m2)
