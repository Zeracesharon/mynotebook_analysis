...
subroutine diabatic_ALE_legacy(u, v, h, tv, BLD, fluxes, visc, ADp, CDp, dt, Time_end, &
  ...
  real :: W_ratio = 0.8
  !real, dimension(SZI_(G),SZJ_(G),SZK_(GV)+1) :: &
  !  Kd_shear_prev, Kv_shear_prev !to store the value before set it to zero.
  ...
  if (CS%double_diffuse) then
    call set_diffusivity(u, v, h, u_h, v_h, tv, fluxes, CS%optics, visc, dt, Kd_int, G, GV, US, &
                         CS%set_diff_CSp, CS%VBF, Kd_extra_T=Kd_extra_T, Kd_extra_S=Kd_extra_S,Waves=Waves)
  else
    call set_diffusivity(u, v, h, u_h, v_h, tv, fluxes, CS%optics, visc, dt, Kd_int, G, GV, US, &
                         CS%set_diff_CSp, CS%VBF,Waves=Waves)
  endif
  call cpu_clock_end(id_clock_set_diffusivity)
...
subroutine diabatic_ALE(u, v, h, tv, BLD, fluxes, visc, ADp, CDp, dt, Time_end, &
  ...
  logical :: showCallTree ! If true, show the call tree
  integer :: i, j, k, is, ie, js, je, isd, ied, jsd, jed, Isq, Ieq, Jsq, Jeq, nz, Kint
  logical :: additive_conflict !if true, shear energy coupling and ePBL addictive option conflicts, error and reset
  real    :: cap_max,cap, zsum !used to evaluate if shear needs to shutdown and the layer where it needs
  ...
  if (CS%use_energetic_PBL) then
    ...
    do K=2,nz ; do j=js,je ; do i=is,ie
      ...
      Kd_add_pos  = max(Kd_add_raw, 0.0)   ! contributes to tendencies + Kd_int
      Kd_add_neg  = min(Kd_add_raw, 0.0)   ! removes shear from Kd_int only
      Kd_add_here = Kd_add_pos  

      Kd_heat(i,j,K) = Kd_heat(i,j,K) + Kd_add_pos+Kd_add_neg
      Kd_salt(i,j,K) = Kd_salt(i,j,K) + Kd_add_pos+Kd_add_neg
    enddo ; enddo ; enddo
...
!    (5) Possibly splits buffer layer into two isopycnal layers (when using isopycnal coordinate)
  ...
  if (CS%double_diffuse) then
    call set_diffusivity(u, v, h, u_h, v_h, tv, fluxes, CS%optics, visc, dt, Kd_int, G, GV, US, &
                         CS%set_diff_CSp, CS%VBF, Kd_lay=Kd_lay, Kd_extra_T=Kd_extra_T, Kd_extra_S=Kd_extra_S,Waves=Waves)
  else
    call set_diffusivity(u, v, h, u_h, v_h, tv, fluxes, CS%optics, visc, dt, Kd_int, G, GV, US, &
                         CS%set_diff_CSp, CS%VBF, Kd_lay=Kd_lay,Waves=Waves)
  endif
  call cpu_clock_end(id_clock_set_diffusivity)
  if (showCallTree) call callTree_waypoint("done with set_diffusivity (diabatic)")
