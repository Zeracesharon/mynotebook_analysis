  ...
  if (CS%use_energetic_PBL) then
    ...
    do j=js,je; do i=is,ie
      ...
      if (cap > 0.0) then
        ...
        do K=2,nz
          z_top_kd = zsum
          z_cen_kd = z_top_kd + 0.5*h(i,j,K-1)
          if (zsum < cap) then
          ...
          zsum = zsum + h(i,j,K-1)    ! consisten with  cap definition: dz, dz(i,j,K-1)
        end do
      end if
    end do; end do
      ...
      if (CS%ePBL_is_additive) then
        ...
        visc%Kv_shear(i,j,K) = visc%Kv_shear(i,j,K) + CS%ePBL_Prandtl*Kd_ePBL(i,j,K)
            ...
            if (associated(visc%Kd_shear) .and. osbl_mask(i,j,K) .and. Kd_int(i,j,K) >= visc%Kd_shear(i,j,K)) then
                Kd_add_here =w_kd(i,j,K)*( Kd_ePBL(i,j,K) -visc%Kd_shear(i,j,K))+ &
...
subroutine diabatic_ALE(u, v, h, tv, BLD, fluxes, visc, ADp, CDp, dt, Time_end, &
  ...
  if (CS%use_energetic_PBL) then
    ...
    KS_cap_m = 0.0    
    call energetic_PBL(h, u_h, v_h, tv, fluxes,visc, dt, Kd_ePBL, G, GV, US, &
                       CS%ePBL, stoch_CS, dSV_dT, dSV_dS, cTKE, SkinBuoyFlux, waves=waves,KS_cap_m_out = KS_cap_m)
    ...
    Kd_shear_prev(:,:,:) = visc%Kd_shear(:,:,:)
    Kv_shear_prev(:,:,:) = visc%Kv_shear(:,:,:)
    osbl_mask(:,:,:) = .false.
    if (associated(visc%Kd_shear)) then
      do j = js, je
        do i = is, ie
        cap = KS_cap_m(i,j)   ! [m] writing by ePBL : 0 means shear shutdown should not be done
          if (cap > 0.0) then
            zsum = 0.0
            do k = 1, nz
              if (zsum >= cap) exit
              Kint = k + 1
              ! 2) set κ-shear to zero for OSBL
              visc%Kd_shear(i,j,Kint) = 0.0
              visc%Kv_shear(i,j,Kint) = 0.0
              ! update zsum for new layer : using  h(i,j,k)or dz(k)
              zsum = zsum + h(i,j,k)
              !zsum = zsum + dz(k)
            end do
          end if
    ...
    do j=js,je; do i=is,ie
       ...
       if (cap > 0.0) then
               zsum = 0.0
               do K=2,nz
                 if (zsum < cap) osbl_mask(i,j,K) = .true.
                 zsum = zsum + h(i,j,K-1) ! consisten with cap definition: dz, dz(i,j,K-1) 
               end do
       end if
    end do; end do               
    ...
    do K=2,nz ; do j=js,je ; do i=is,ie
      if (CS%ePBL_is_additive) then
        ...
        visc%Kv_shear(i,j,K) = visc%Kv_shear(i,j,K) + CS%ePBL_Prandtl*Kd_ePBL(i,j,K)
           ...
           if (associated(visc%Kd_shear) .and. osbl_mask(i,j,K)) then
                Kd_add_here = max(Kd_ePBL(i,j,K) - Kd_shear_prev(i,j,K),0.0)
                visc%Kv_shear(i,j,K) = CS%ePBL_Prandtl*Kd_ePBL(i,j,K)
           else
                Kd_add_here = max(Kd_ePBL(i,j,K) - visc%Kd_shear(i,j,K), 0.0)
                visc%Kv_shear(i,j,K) = max(visc%Kv_shear(i,j,K), CS%ePBL_Prandtl*Kd_ePBL(i,j,K))
           endif
