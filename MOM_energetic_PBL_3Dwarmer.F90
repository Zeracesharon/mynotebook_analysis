...
type, public :: energetic_PBL_CS ; private
  ...
  logical :: use_shear_energy_coupling = .false.  ! 开关：启用能量耦合
  logical :: shutdown_shear_in_ML      = .true.   ! 当 shear 能量主导时，关停 MLD 内的 Kd_shear
  logical :: warn_if_no_kappa_shear    = .true.   ! ePBL 开启耦合而 kappa_shear 未开时给提示
  logical :: LT_rescale_by_shear = .false.   ! EPBL_LT_RESCALE_BY_SHEAR
  real    :: LT_enhance_top = 1.0
  character(len=32) :: Shear_LT_mode = 'preserve_fraction'
  real    :: LT_rescale_gamma    = 0.5       ! EPBL_LT_RESCALE_GAMMA (recommended 0.2–0.5)
  ...
  real :: Shear_alpha           =-1! <0 => auto (Ri-based); >=0 => prescribed alpha in [0,1]
  logical :: ShearEnergy_Hbl_est = .false. !True for using largest dT/dt for MLD when calculat ME for shear energy otherwise MLD_in
...
!      mstar = 1.25, nstar = 0.4, TKE_decay = 0.0, conv_decay = 0.0
  ...
  do j=js,je
    ...
    do i=is,ie ; if (G%mask2dT(i,j) > 0.0) then
      ...
      ml_cap = max(0.0, min(eCD%Hbl_est, sum(dz)))
...
! interface.
  ...
  do OBL_it=1,CS%Max_MLD_Its
    ...
    !/ Here we get MStar, which is the ratio of convective TKE driven mixing to UStar**3
      ! First choose path: original vs energy-coupled (runtime switch)
    if (.not. CS%use_shear_energy_coupling) then
        ...
        eCD%used_shear_flag = 0.0
         ...
         if (present(use_shear_energy) .and. use_shear_energy .and. &
             ...
             if (.not. CS%ShearEnergy_Hbl_est) then
               if (present(Kd_shear_col)) then
                    if (CS%Shear_alpha > 0.0) then
                         ...
                         a_lo  = 0.85 ; a_hi = 1.00    ! gentle shrink only
                    ...
                    eCD%Hbl_est = abs(MLD_guess) !Hbl_est   !roughly estimate of MLD rewritten by MLD_guess
               endif
             endif
             ...
             eCD%mstar_shear  = mstar_shear
               msafe = eCD%mstar_shear
                mstar_shear_ok = ((msafe == msafe) .and. (abs(msafe) < 1.0e8) .and. (msafe > 0.0) .and.  &
                 ( E_shear<(E_ePBL_local*1.0e8) ))
                hmin = dz(1)           ! 也可以用 3.0*dz(1) 更保守
                thick_ok = (abs(Hbl_est) >= hmin)
             if ((E_shear > E_ePBL_local) .and. (u3 > 1.0e-6) .and. mstar_shear_ok .and. thick_ok) then
                 mstar_total            =  mstar_shear
                 used_shear_flag        =  1.0
             else
       ...
       !Calculate mstar_LT based on mstar_total that has been potential replaced by shear energy
         if (CS%Use_LT) then
             call get_Langmuir_Number(LA, G, GV, US, abs(MLD_guess), u_star_mean, i, j, dz, Waves, &
                                      U_H=u, V_H=v)
          ! --- reference LT on ePBL base to form fraction  (no shear rescale) ---
             mstar_LT0 = 0.0
             ...
             if (CS%LT_rescale_by_shear) then
                 ...
                 case ('energy_gamma')
                     E_used  = max(E_ePBL_local, E_shear)
                     denom   = max(E_ePBL_local, 1.0e-12)
                     ratio_lt=E_used/denom
                     if (ratio_lt > 0.0) then
                       lt_scale = max(1.0, ratio_lt**CS%LT_rescale_gamma)
                       if (.not.(lt_scale .eq. lt_scale)) lt_scale = 1.0   ! if NaN, fall back
                       lt_scale = min(lt_scale, 1.0e10)        ! optional ceiling to prevent runaways
                     endif
                     if ((used_shear_flag < 0.5) .or. &
                     (mstar_e_local < 1.0e-4) .or. (mstar_total < 1.0e-4)) then
                        lt_scale=1
                     endif
                 ...
                 case ('preserve_fraction')     !keep the mstar_LT/(mstar_ePBL) ratio the same
                     scale_need= 1.0
                           if (eCD%used_shear_flag > 0.5 .and. mstar_total > 1.0e-5 ) then
                              scale_need = ( mstar_total / max( max(mstar_e_local, &
                                      0.0001*mstar_total), 1.0e-5) ) ** CS%LT_rescale_gamma     
                           endif
                           if (.not.(scale_need .eq. scale_need)) scale_need = 1.0
                           if (scale_need < 1.0) scale_need = 1.0                                  ! Rescale_LT must be >= 1
                           if (scale_need > 1.0e4) scale_need = 1.0e4
...
!/2. Options related to setting MSTAR
  ...
  call get_param(param_file, mdl, "W_RATIO", CS%W_ratio, &
          "when apply weighted function on Kd_interface for smooth profile,top ratio of ML keeps unchange", &
                 ...
                 do_not_log=(CS%mstar_scheme==Use_Fixed_MStar .and. (.not. CS%use_shear_energy_coupling)))
  call get_param(param_file, mdl, "SHEARENERGY_HBL_EST", CS%ShearEnergy_Hbl_est, &
               "If false use ePBL MLD for calculating shear energy"\
       "If true, use largest temperature derivative for shear energy ME", &
...
!/ Turbulent velocity scale in mixing coefficient
  ...
  call get_param(param_file, mdl, "EPBL_LT_RESCALE_GAMMA", CS%LT_rescale_gamma, &
               ...
               "and in 'preserve_fraction' as (target_full/mLTtmp)^gamma. (0.3–0.8 recommended).", &
                 units="nondim", default=0.5, &
                 do_not_log=(.not. (CS%Use_LT .and. CS%use_shear_energy_coupling)))
  if (CS%LT_rescale_by_shear .and. (CS%LT_rescale_gamma < 0.2 .or. CS%LT_rescale_gamma > 0.8)) then
