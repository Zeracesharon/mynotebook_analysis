...
! mixing.
  ...
  real :: mstar_LT0, f0,mtmp, mLTtmp, target_full, scale_need,base0,beta
           ...
           if (CS%Use_LT) then
            ...
            ! --- reference LT on ePBL base to form fraction f0 (no shear rescale) ---
               ...
               call mstar_Langmuir(CS, US, absf, B_flux, u_star, MLD_guess, La, &
                                   mstar_e_local, mstar_LT0, LAmod)
               f0 = mstar_LT0 / max(mstar_e_local, 1.0e-12)
               lt_scale = 1.0
               if (CS%LT_rescale_by_shear) then
                   if (CS%Shear_LT_mode == 'energy_gamma') then
                       E_used  = max(E_ePBL_local, E_shear)
                       denom   = max(E_ePBL_local, 1.0e-12)
                       ratio_lt=E_used/denom
                       if (ratio_lt .le. 0.0) then
                         lt_scale = 1.0
                       else
                   ...
                   else if (CS%Shear_LT_mode == 'preserve_fraction') then     !keep the mstar_LT/(mstar_LT+mstar_ePBL) ratio the same
                       base0  = mstar_total
                       mtmp   = base0
                       mLTtmp = 0.0
                       call mstar_Langmuir(CS, US, absf, B_flux, u_star, MLD_guess, La, &
                            mtmp, mLTtmp, LAmod)               ! no Rescale_LT
                       target_full = ( f0 / max(1.0 - f0, 1.0e-6) ) * base0
                       beta = E_ePBL_local / max( max(E_ePBL_local, E_shear), 1.0e-12 )
                       scale_need = 1.0
                       if (mLTtmp > 0.0 .and. target_full > 0.0) scale_need = (target_full / mLTtmp)**CS%LT_rescale_gamma
                       if (.not.(scale_need .eq. scale_need)) scale_need = 1.0
                       if (scale_need < 1.0) scale_need = 1.0                                  ! Rescale_LT must be >= 1
                       scale_need = min(scale_need, 1.0e10)
                       call mstar_Langmuir(CS, US, absf, B_flux, u_star, MLD_guess, La, &
