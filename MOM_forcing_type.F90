...
subroutine find_ustar_fluxes(fluxes, tv, U_star, G, GV, US, halo, H_T_units,nx_wind,ny_wind)
  ...
  ! optional wind/stress direction unit vectors (H grid)
  real, dimension(SZI_(G),SZJ_(G)), optional, intent(out) :: nx_wind, ny_wind
  ...
  real :: angle_w  !for wind direction
  ...
  if (present(nx_wind) .and. present(ny_wind)) then
    ...
    ! 1) Use stress components if present
    if (associated(fluxes%omega_w2x)) then
      do j=js,je ; do i=is,ie
        angle_w  = fluxes%omega_w2x(i,j)
        nx_wind(i,j) = cos(angle_w)
        ny_wind(i,j) = sin(angle_w)
      enddo ; enddo
    end if
  end if
...
!> Determine the friction velocity from the contenxts of a forcing type, perhaps
!! using the evolving surface density.
subroutine find_ustar_mech_forcing(forces, tv, U_star, G, GV, US, halo, H_T_units, nx_wind,ny_wind)
  type(ocean_grid_type),   intent(in)  :: G    !< The ocean's grid structure
  type(verticalGrid_type), intent(in)  :: GV   !< The ocean's vertical grid structure
  type(unit_scale_type),   intent(in)  :: US   !< A dimensional unit scaling type
  ...
  logical :: Z_T_units ! If true, U_star is returned in units of [Z T-1 ~> m s-1], otherwise it is
                       ! returned in [H T-1 ~> m s-1 or kg m-2 s-1]
  integer :: i, j, k, is, ie, js, je, hs
  real :: tx, ty, mag  !for wind direction
  ...
  if (present(nx_wind) .and. present(ny_wind)) then
    ...
    ! 1) Use stress components if present
    if (associated(forces%taux) .and. associated(forces%tauy)) then
      do j=js,je ; do i=is,ie
        tx  = forces%taux(i,j)
        ty  = forces%tauy(i,j)
