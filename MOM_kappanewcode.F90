...
! mixing.
  ...
  real :: E_shear, u3, mstar_e_local, E_ePBL_local,Me_shear_loc, Conv_shear_loc,Hbl_est
  real :: lt_scale, E_used, denom, ratio_lt !where LT will be rescaled based on energy         
  real :: mstar_LT0,tmp_e_local, scale_need,base0
    ...
    if (.not. CS%use_shear_energy_coupling) then
        if (CS%Use_LT) then
         ...
         if (present(use_shear_energy) .and. use_shear_energy .and. &
             ...
             eCD%mstar_shear  = E_shear / max(u3, tiny(1.0))
             if ((E_shear > E_ePBL_local) .and. (u3 > 1.0e-6)) then
          ...
          ! --- reference LT on ePBL base to form fraction  (no shear rescale) ---
             ...
             if (CS%LT_rescale_by_shear) then
                 ...
                 case ('energy_gamma')
                     ...
                     if ((eCD%used_shear_flag < 0.5) .or. &
                     (mstar_e_local < 1.0e-4) .or. (mstar_total < 1.0e-4) .or.  (mstar_LT0 > 0.8 * mstar_total)) then
                        lt_scale=1
                     endif
                 ...
                 case ('preserve_fraction')     !keep the mstar_LT/(mstar_ePBL) ratio the same
                     base0 = eCD%mstar_shear / max(eCD%mstar_ePBL, 1.0e-4)
                     scale_need = 1.0
                     if (base0 > 1.05) scale_need = base0**CS%LT_rescale_gamma
                     if (.not.(scale_need .eq. scale_need)) scale_need = 1.0
                     if (scale_need < 1.0) scale_need = 1.0                                  ! Rescale_LT must be >= 1
                     scale_need = min(scale_need, 1.0e5)
                     if ((eCD%used_shear_flag < 0.5) .or. (mstar_e_local < 1.0e-4) .or. (mstar_total < 1.0e-4) .or. &
                             (mstar_LT0 > 0.8 * mstar_total)) scale_need = 1.0
...
subroutine Mstar_Langmuir(CS, US, Abs_Coriolis, Buoyancy_Flux, UStar, BLD, Langmuir_Number, &
  ...
  mstar    = m0*enhance_mstar + mstar_LT
...
!/2. Options related to setting MSTAR
  ...
  if (CS%Shear_alpha >= 0.0) then
    if (CS%Shear_alpha > 1.0) then
      call MOM_mesg('SHEAR_ALPHA > 1; clamping to 1.0', 0)
    endif
    if (CS%Shear_alpha < 0.0) then
      call MOM_mesg('SHEAR_ALPHA < 0; using Ri-based alpha instead', 0)
    endif
    CS%Shear_alpha = max(0.0, min(1.0, CS%Shear_alpha))
  endif
