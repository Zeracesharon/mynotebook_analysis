...
subroutine set_diffusivity(u, v, h, u_h, v_h, tv, fluxes, optics, visc, dt, Kd_int, &
                           G, GV, US, CS, VBF, Kd_lay, Kd_extra_T, Kd_extra_S,Waves)
  ...
  type(wave_parameters_CS), pointer, optional, intent(in) :: Waves
  ...
  elseif (CS%useGOTM) then   
    ...
    if (CS%Vertex_shear) then
      call GOTM_calculate_vertex(CS%GOTM_CSp, G, GV, Dt, h, tv%T, tv%S, u, v, tv%eqn_of_state, &
           fluxes%ustar, visc%TKE_turb, visc%TKE_diss, visc%TKE_ML, visc%Kd_shear,Kd_tmp, visc%Kv_shear_Bu,Waves )
      if (associated(visc%Kv_shear)) visc%Kv_shear(:,:,:) = 0.0 ! needed for other parameterizations
    else                   
      call GOTM_calculate(CS%GOTM_CSp, G, GV, Dt, h, tv%T, tv%S, u, v, tv%eqn_of_state, &
           fluxes%ustar, visc%TKE_turb, visc%TKE_diss, visc%TKE_ML, visc%Kd_shear, Kd_tmp, visc%Kv_shear,Waves )
    endif    
