...
type, public :: wave_parameters_CS ; private
  ...
  real, allocatable, dimension(:,:,:) :: &
    STKy0              !< Stokes Drift spectrum (meridional) [L T-1 ~> m s-1]
                       ...
                       !! 3rd dimension -> Freq/Wavenumber
  real, allocatable, dimension(:,:,:) :: &
     dUsdz_theory_x, dUsdz_theory_x_T  !x direction stokes shear profile on C grid and T grid
  real, allocatable, dimension(:,:,:) :: &
     dVsdz_theory_y, dVsdz_theory_y_T  !y direction stokes shear profile on C grid and T grid
  ...
  integer :: id_dUsdz_theory_x = -1, id_dUsdz_theory_x_T = -1
  integer :: id_dVsdz_theory_y = -1, id_dVsdz_theory_y_T = -1
  !>@}
...
# include "version_variable.h"
  ...
  allocate(CS%dUsdz_theory_x(G%isdB:G%IedB, G%jsd:G%jed,  GV%ke), source=0.0)
  allocate(CS%dVsdz_theory_y(G%isd:G%Ied,   G%jsdB:G%jedB, GV%ke), source=0.0)
!  allocate(CS%dUsdz_theory_x_T(G%isc:G%iec, G%jsc:G%jec, GV%ke), source=0.0)
!  allocate(CS%dVsdz_theory_y_T(G%isc:G%iec, G%jsc:G%jec, GV%ke), source=0.0)  
  allocate(CS%dUsdz_theory_x_T(SZI_(G), SZJ_(G), SZK_(G)+1),source=0.0) ; 
  allocate(CS%dVsdz_theory_y_T(SZI_(G), SZJ_(G), SZK_(G)+1),source=0.0) ; 
  ...
  CS%id_dUsdz_theory_x = register_diag_field('ocean_model', 'stokes_shear_x_theory', &
       CS%diag%axesCuL,Time, 'theoretical Stokes vertical shear (x), LF17','s-1')
  CS%id_dVsdz_theory_y = register_diag_field( 'ocean_model', 'stokes_shear_y_theory',&
       CS%diag%axesCvL,Time, 'theoretical Stokes vertical shear (y), LF17','s-1')
  CS%id_dUsdz_theory_x_T = register_diag_field( 'ocean_model','stokes_shear_x_theory_T',&
       CS%diag%axesTi,Time, 'theoretical Stokes vertical shear (x) on T grid, LF17','s-1')
  CS%id_dVsdz_theory_y_T = register_diag_field( 'ocean_model', 'stokes_shear_y_theory_T', &
       CS%diag%axesTi,Time,'theoretical Stokes vertical shear (y) on T grid, LF17','s-1')
  if (CS%Stokes_DDT) then
...
subroutine Update_Stokes_Drift(G, GV, US, CS, dz, ustar, dt, dynamics_step,nx_wind,ny_wind)
  ...
  ! --- for LF17 ---
  real, allocatable :: z_srf(:), UStokes_SL_ifc(:),dUdz_ifc(:),Us_ifc(:)
  real :: U10, La_tmp, nx, ny, tmp, z_srf0, Left,Right,tmp_th,denu,denv
  ...
  elseif (CS%WaveMethod==LF17) then
     ...
     if (.not. allocated(dUdz_ifc))       allocate(dUdz_ifc(0:GV%ke))
     if (.not. allocated(Us_ifc))         allocate(Us_ifc(0:GV%ke))     
     ...
     do j = G%jsc, G%jec
       do I = G%iscB, G%iecB
         ...
         !    Left=max(G%isc, I-1) 
          !   Right= min(G%iec, I)  
          !     nx  = 0.5*( nx_wind(left,j) + nx_wind(right,j) )
           !    ny  = 0.5*( ny_wind(left,j) + ny_wind(right,j) ) 
                nx = nx_wind(I,j)
                ny = ny_wind(I,j)
           end if
         !!to calculate the theoretical stokes drift shear profile instead of using finite difference
         dUdz_ifc(0) = (UStokes_SL_ifc(1) - UStokes_SL_ifc(0)) / max(1.0e-12, z_srf(1) - z_srf(0))
         do kk = 1, GV%ke-1
           dUdz_ifc(kk) = (UStokes_SL_ifc(kk+1) - UStokes_SL_ifc(kk-1)) / &
                          max(1.0e-12, z_srf(kk+1) - z_srf(kk-1))
         end do
         dUdz_ifc(GV%ke) = (UStokes_SL_ifc(GV%ke) - UStokes_SL_ifc(GV%ke-1)) / &
                           max(1.0e-12, z_srf(GV%ke) - z_srf(GV%ke-1))
         
         ! 2) Point Stokes drift at interfaces
         do kk = 0, GV%ke
           Us_ifc(kk) = UStokes_SL_ifc(kk) - z_srf(kk) * dUdz_ifc(kk)
         end do
         
         ! 3) Layer-mean theoretical shear on u-points (match the direction scaling with nx)
         do k = 1, GV%ke
           tmp_th = ( Us_ifc(k-1) - Us_ifc(k) ) / max(1.0e-12, z_srf(k) - z_srf(k-1))
           CS%dUsdz_theory_x(I,j,k) = tmp_th * nx      ! units s-1
         end do
      ...
      do J = G%jscB, G%jecB
        do i = G%isc, G%iec
          ...
          nx = 1.0 ; ny = 0.0
          if (present(nx_wind) .and. present(ny_wind)) then
          !   Bottom = max(G%jsc, J-1)                 ! “below” H cell
          !   Top = min(G%jec, J)                   ! “above”  H cell
          !   nx  = 0.5*( nx_wind(i,Bottom) + nx_wind(i,Top) )
          !   ny  = 0.5*( ny_wind(i,Bottom) + ny_wind(i,Top) )
         ...
         do k = 1, GV%ke
           tmp_th = ( Us_ifc(k-1) - Us_ifc(k) ) / max(1.0e-12, z_srf(k) - z_srf(k-1))
           CS%dVsdz_theory_y(i,J,k) = tmp_th * ny    ! match your sign convention for v/Stokes_y
         end do
     ...
     !average C grid to obtain data on T grid
     ! Interior cells where i-1 and j-1 exist
!     do j = G%jsc+1, G%jec
!       do i = G%isc+1, G%iec
!         do k = 1, GV%ke
!           CS%dUsdz_theory_x_T(i,j,k) = 0.5 * ( CS%dUsdz_theory_x(i-1,j,k) + CS%dUsdz_theory_x(i,j,k) )
!           CS%dVsdz_theory_y_T(i,j,k) = 0.5 * ( CS%dVsdz_theory_y(i,  j-1,k) + CS%dVsdz_theory_y(i,  j,k) )
!         end do
!       end do
!     end do
!     
!     ! Fill the first column/row by copying the nearest interior value (still halo-free)
!     CS%dUsdz_theory_x_T(G%isc,:,:) = CS%dUsdz_theory_x_T(G%isc+1,:,:)
!     CS%dVsdz_theory_y_T(:,G%jsc,:) = CS%dVsdz_theory_y_T(:,G%jsc+1,:)
     ! Average u-face shear to T points
     do j = G%jsc, G%jec
       do i = G%isc, G%iec
         if (G%mask2dT(i,j) > 0.5) then
           do k = 1, GV%ke
             ! u-faces → T (left I=i-1, right I=i), clamp with *_B halos
             im1  = max(G%iscB, i-1)
             denu = G%mask2dCu(im1,j) + G%mask2dCu(i,j)
             if (denu > 0.0) then
               CS%dUsdz_theory_x_T(i,j,k) = ( &
                    CS%dUsdz_theory_x(im1,j,k)*G%mask2dCu(im1,j) + &
                    CS%dUsdz_theory_x(i,  j,k)*G%mask2dCu(i,  j) ) / denu
             else
               CS%dUsdz_theory_x_T(i,j,k) = 0.0
             end if
     
             ! v-faces → T (bottom J=j-1, top J=j)
             jm1  = max(G%jscB, j-1)
             denv = G%mask2dCv(i,jm1) + G%mask2dCv(i,j)
             if (denv > 0.0) then
               CS%dVsdz_theory_y_T(i,j,k) = ( &
                    CS%dVsdz_theory_y(i,jm1,k)*G%mask2dCv(i,jm1) + &
                    CS%dVsdz_theory_y(i,j,  k)*G%mask2dCv(i,j)   ) / denv
             else
               CS%dVsdz_theory_y_T(i,j,k) = 0.0
             end if
           end do
         else
           CS%dUsdz_theory_x_T(i,j,:) = 0.0
           CS%dVsdz_theory_y_T(i,j,:) = 0.0
         end if
       end do
     end do
      ...
      call pass_vector(CS%Us_x(:,:,:), CS%Us_y(:,:,:), G%Domain, To_All)
      call pass_vector(CS%Us0_x(:,:),  CS%Us0_y(:,:),  G%Domain)
      call pass_vector(CS%dUsdz_theory_x(:,:,:), CS%dVsdz_theory_y(:,:,:), G%Domain, To_All)
      call pass_var(CS%dUsdz_theory_x_T, G%Domain, To_All)
      call pass_var(CS%dVsdz_theory_y_T, G%Domain, To_All)
  ...
  if (CS%id_dUsdz_theory_x > 0) &
    call post_data(CS%id_dUsdz_theory_x, CS%dUsdz_theory_x, CS%diag)
  if (CS%id_dVsdz_theory_y > 0) &
    call post_data(CS%id_dVsdz_theory_y, CS%dVsdz_theory_y, CS%diag)
  if (CS%id_dUsdz_theory_x_T   > 0) &
    call post_data(CS%id_dUsdz_theory_x_T,   CS%dUsdz_theory_x_T,   CS%diag)
  if (CS%id_dVsdz_theory_y_T   > 0) &
    call post_data(CS%id_dVsdz_theory_y_T,   CS%dVsdz_theory_y_T,   CS%diag)
  if (CS%Stokes_DDT) then
...
subroutine get_StokesSL_LiFoxKemper(ustar, hbl, GV, US, CS, UStokes_SL, LA,u10_in)
