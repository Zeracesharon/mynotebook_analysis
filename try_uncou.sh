...
NUM_TOT=20  ###overall duration/months
...
#  rm restart*.ww3
  ...
  rm time*out
  cd INPUT/
  rm coupler.res ice_model.res.nc MOM.res_1.nc MOM.res.nc
  cd ../
else
...
#	../../../../build/ncrc6.intel23/ww3_ounf/REPRO/ww3_ounf
...
#	sed -i "8s/.*/${newline_ww3}/" "ww3_multi.inp"
	...
	end_date_ww3=$(date -d "$start_date_ww3 +$number_month months +$number_day days" +"%Y-%m-%d")
...
#	start_sec=$(date -d "$start_date_ww3" +%s)
#	end_sec=$(date -d "$end_date_ww3" +%s)
